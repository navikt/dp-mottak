                        ...
                        when (saksbehandlingKlient.skalVarsleOmEttersending(søknadId = søknadId, ident = ident)) {
                            true ->
                                runBlocking {
                                    ...
                                    logger.info { "Lager gosys-oppgave for å varsle om ettersending" }
                                    sikkerlogg.info { "Oppretter gosys-oppgave: $oppgave for søknadId: $søknadId og ident: $ident" }
                                    gosysClient.opprettOppgave(oppgave = oppgave).also {
                                        sikkerlogg.info { "Opprettet gosys oppgave for søknadId: $søknadId og ident: $ident og gosys id: $it " }
                                    }
                                }

                            else -> {
                                logger.info { "Lager IKKE gosys-oppgave for å varsle om ettersending til søknadId: $søknadId journalpostId: ${event.journalpostId}" }
                            }
                        }
                    }
                }
